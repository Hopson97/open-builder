    ...
    , m_worldSize(config.worldSize)
{
    ...
    */
    ...
    {
        LOGVAR("Server", "Received player skin, packet size: ",
        ...
        // player skin data
        sf::Uint8 *skinPixels = (sf::Uint8 *)packet.getData() +
                                sizeof(command_t) + sizeof(peer_id_t);
    ...
    void Server::removePeer(u32 connectionId)
    {
        ...
        if (itr != m_connectedClients.cend()) {
            LOGVAR("Server",
                   "Client disconnected, Peer Id:", (int)itr->entityId);
